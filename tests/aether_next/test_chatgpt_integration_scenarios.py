...
from proteus.aether_next.integration_scenarios import (
    run_disabled_tool_guard_scenario,
    run_workbench_verifier_repair_scenario,
)
from scripts.aether_next.run_verifier_only_eval import _case_semantic_wrong, _env as _verifier_env, _model_messages
...
def test_deterministic_integration_runner_writes_auditable_bundle(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        [sys.executable, str(_BUILD_ROOT / "scripts" / "aether_next" / "run_deterministic_integration_eval.py"), "--out-dir", str(out_dir)],
        cwd=_BUILD_ROOT,
        check=True,
        text=True,
...
def test_verifier_only_validator_accepts_fake_bundle_and_writes_report(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [sys.executable, str(_BUILD_ROOT / "scripts" / "aether_next" / "run_verifier_only_eval.py"), "--mode", "fake", "--out-dir", str(out_dir)],
        cwd=_BUILD_ROOT,
        check=True,
        text=True,
        stdout=subprocess.PIPE,
    ...
    proc = subprocess.run(
        [sys.executable, str(_BUILD_ROOT / "scripts" / "aether_next" / "validate_verifier_only_eval.py"), str(out_dir), "--report", str(report)],
        cwd=_BUILD_ROOT,
        check=True,
        text=True,
        stdout=subprocess.PIPE,
...
def test_verifier_only_validator_rejects_parse_error_bundle(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        [sys.executable, str(_BUILD_ROOT / "scripts" / "aether_next" / "validate_verifier_only_eval.py"), str(out_dir)],
        cwd=_BUILD_ROOT,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
