...
import proteus.aether_next as aether_next
from proteus.aether_next.run_adapter import ensure_certified_architect_mode
...
def test_certified_package_has_no_reference_legacy_imports() -> None:
    ...
    assert not offenders, f"certified modules import older_variants.reference_legacy: {offenders}"
...
def test_importing_all_certified_modules_never_loads_reference_legacy() -> None:
    ...
    code = (
        ...
        "import proteus.aether_next as aether_next\n"
        "for info in pkgutil.walk_packages(aether_next.__path__, 'proteus.aether_next.'):\n"
        "    importlib.import_module(info.name)\n"
        "loaded = [name for name in sys.modules if name.startswith('older_variants.reference_legacy')]\n"
        "assert not loaded, f'older_variants.reference_legacy loaded: {loaded}'\n"
        "legacy = [name for name in sys.modules if any(\n"
        "    name == f'proteus.aether_next.{q}' for q in (\n"
    ...
    proc = subprocess.run(
        ...
        capture_output=True,
        text=True,
        cwd=str(_CERTIFIED_ROOT.parents[1]),
        timeout=120,
    )
...
def test_reference_legacy_still_importable_as_reference() -> None:
    ...
    module = importlib.import_module("older_variants.reference_legacy.proof_contract")
