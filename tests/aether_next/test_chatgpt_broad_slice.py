...
from proteus.aether_next.kernel_config import resolve_runtime
...
def test_verifier_only_fake_runner_writes_evidence_bundle(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [sys.executable, str(_BUILD_ROOT / "scripts" / "aether_next" / "run_verifier_only_eval.py"), "--mode", "fake", "--out-dir", str(out)],
        check=True,
        text=True,
        capture_output=True,
        cwd=_BUILD_ROOT,
