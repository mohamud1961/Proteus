...
from proteus.aether_next import (
    EvidenceRecord,
    HarnessRuntime,
    OutcomeOwner,
    RouteAction,
...
def test_verifier_activation_uses_fallback_route_once(contract, config_factory):
    from proteus.aether_next.config import compile_workbench_config
...
def test_legacy_completed_cannot_bypass_evidence_contract():
    from proteus.aether_next import adapt_legacy_verdict
...
def test_executable_verifier_recovery_uses_fallback_after_primary_tool_failure(contract, config_factory):
    from proteus.aether_next import execute_clause_inspection_with_recovery
    from proteus.aether_next.config import compile_workbench_config
