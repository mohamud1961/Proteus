...
def _has_forbidden_access_intent(command: str) -> bool:
    ...
    for token in FORBIDDEN_TOKENS:
        ...
        if token_fragments and all(
            _is_exclusion_only_fragment(frag, token) for frag in token_fragments
        ):
            continue
        return True
    return False
...
def _classify(reason_codes, fallback):
    mapping = {
        "move_content_mismatch": "retrieval_reduction",
        "move_count_mismatch": "retrieval_reduction",
        "missing_move": "retrieval_reduction",
        "extra_move": "retrieval_reduction",
        "missing_trailing_newline": "retrieval_reduction",
        "missing_moves_txt": "retrieval_reduction",
...
def main() -> int:
    ...
    if reason_codes:
        ...
        failure_class = _classify(
            reason_codes,
            base.get("failure_class", "retrieval_reduction"),
        )
