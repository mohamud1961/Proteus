...
def architecture_purity(build_root: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    del manifest
    package = build_root / "proteus" / "aether_next"
...
def official_task_board_integrity(build_root: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    del manifest
    path = build_root / "eval_suite" / "evals" / "official_task_board.v1.json"
    findings: list[str] = []
    try:
...
def deterministic_system_scenarios(build_root: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    from proteus.aether_next.integration_scenarios import run_all_integration_scenarios
...
def archetype_matrix_coverage(build_root: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    """
    board_path = build_root / "eval_suite" / "evals" / "official_task_board.v1.json"
    matrix_path = build_root / "eval_suite" / "evals" / "archetype_matrix.v1.json"
    try:
