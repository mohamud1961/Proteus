...
The source record distinguishes implementation commits from evidence-only commits and preserves negative classifications. In particular, fresh certification evidence and model-board evidence are not silently upgraded into a public product score. The Proteus package is the full public source boundary derived from that authority: it includes the Proteus runtime, runner, public eval substrate and fixtures, tests, and scripts. The `older_variants/` directory contains the Aether-2 harness, `aether` alias, and `reference_legacy` package, all explicitly marked old/reference-only surfaces and are not the current Proteus route. Private VM/model artifacts and benchmark payloads are intentionally excluded.
